    ...
    <el-row :gutter="15" style="font-size:12px; margin:0 5px; height:calc(100% - 115px)">
        <el-col :xs="24" :sm="24" class="h100">
            <div class="chartheader">
                <span>部门选择</span>
                <el-select size="mini" style="margin-right:20px" v-model="dept">
                    <el-option v-for="(item,i) in depts" :key="i" :value="item.id+'|'+item.name" :label="item.name"></el-option>
                </el-select>
                        ...
                        <div class="circlelist h100">
                            <div>
                                <div class="tcircle center circle">
                                  ...
                                  {{Math.abs(sum1-sum3)}}</span>
                                    </div>
                                </div>
                                <div class="center hcircle circle">
                                    <span>环比<i v-show="sum1>sum2" class="iconfont icon-icon-arrow-top4" style="color:red;"></i>
                                  <i v-show="sum1<sum2" class="iconfont icon-icon-arrow-btm4" style="color:green;"></i>{{Math.abs(sum1-sum2)}}</span>
                                </div>
                            </div>
                            <div>
                ...
                <el-col :xs="24" :sm="12" :lg="12" class="h100">
                    <el-card shadow="never">
                        ...
                        <el-table :data="tableData" style="width: 100%;" height="100%" show-summary class="eltable">
                            ...
                            <el-table-column prop="val" label="能耗值"></el-table-column>
                        </el-table>
                    </el-card>
                </el-col>
                <el-col :xs="24" :sm="12" :lg="12" class="h100">
...
<script>
import { areaBarOption, areaPieOption } from "@/lib/chartConfig";
export default {
    name: "areaEnergy",
    data() {
        return {
    ...
    methods: {
        loadCategorys() {
            this.$ajax
                .get(this.$URL + "/energy/GetAllDepts")
                .then(result => {
        ...
        updatedPieChart(result) {
            ...
            option.title[0].subtext=sum.toString();
        ...
        showTable(result) { //最大值 最小值 平局值 计算
            ...
            result.data.data1.forEach((v, i) => {
                let obj = {
                    ...
                    val: v,
                    categoryname:this.dept?this.dept.split('|')[1]:'',
                };
                if (_this.max < v) {
                    _this.max = v;
                }
        ...
        loadDeptEnergys() {
            let _this = this;
            this.$ajax
                .get(this.$URL + "/energy/GetDeptEnergy", {
                    params: {
                        bt: this.startDate.format("yyyy-MM-dd") + " 00:00:00",
                        deviceType: this.devicetype,
                        dateType: this.datetype,
                        categoryId: this.dept.split('|')[0]
                    }
                })
                .then(result => {
                    if (result.status === 200 && result.data.success) {
        ...
        loadAllDeptsEnergys() {
            let _this = this;
            this.$ajax
                .get(this.$URL + "/energy/GetAllDeptEnergy", {
                    params: {
                        bt: this.startDate.format("yyyy-MM-dd") + " 00:00:00",
                        deviceType: this.devicetype,
                        dateType: this.datetype
