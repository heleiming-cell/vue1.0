...
<el-row class="h100">
    <el-col :xs="24" :sm="3" class="h100" style="border-right: 1px solid #122d62">
        <div class="menuheader">
            <div style="float:left"><span>部门树</span></div>
            <div style="float:right">
                <span @click="addArea" class="pointer"><i class="iconfont icon-jia"></i></span>
                <span @click="deleteDept" class="pointer" style="margin:0 5px"><i class="iconfont icon-shanchu"></i></span>
                <span @click="editDept" style="margin:0 5px" class="pointer"><i class="iconfont icon-bianji"></i></span>
            </div>
        </div>
    ...
    <el-dialog title="部门树编辑" :visible.sync="deptDialog" :modal-append-to-body="false" width="300px">
    ...
    <el-dialog title="设备选择" :visible.sync="deviceDialog" :modal-append-to-body="false" width="300px">
        ...
        <span slot="footer" class="dialog-footer" style="text-align:center;">
            <el-button type="primary" @click="addDeptDevices">确 定</el-button>
        </span>
    </el-dialog>
...
export default {
    ...
    methods: {
        loadDevices() {
            this.$ajax.get(this.$URL + '/energy/GetDeptDevices', {
                params: {
                    deptId: this.node ? this.node.key : 0,
        ...
        deleteDept() {
            ...
            if (ids.length > 0) {
                this.$confirm('此操作将永久删除改能耗部门, 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    this.$ajax.get(this.$URL + '/energy/DeleteDepts', {
                        params: {
                            id: ids
                        }
                    }).then((result) => {
        ...
        saveDept() {
            ...
            this.$ajax.post(this.$URL + '/energy/AddDept', {
        ...
        addDeptDevices() {
            ...
            this.$ajax.get(this.$URL + '/energy/AddDeptDevices', {
                params: {
                    id: this.selectDevices,
                    deptId: this.node.key,
                    code: this.node.code
                }
            }).then((result) => {
                if (result.status === 200 && result.data.success) {
        ...
        deleteAreaDevice() {
            ...
            this.$ajax.get(this.$URL + '/energy/DeleteDeptDevice', {
                params: {
                    id: this.deleteid
                }
            }).then((result) => {
