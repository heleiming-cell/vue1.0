...
<el-row class="h100">
    <el-col :xs="24" :sm="3" class="h100" style="border-right: 1px solid #122d62">
        <div class="menuheader">
            ...
            <div style="float:right">
                ...
                <span @click="deleteArea" class="pointer" style="margin:0 5px"><i class="iconfont icon-shanchu"></i></span>
                <span @click="editArea" style="margin:0 5px" class="pointer"><i class="iconfont icon-bianji"></i></span>
            </div>
        </div>
    ...
    <el-dialog title="区域树编辑" :visible.sync="areaDialog" :modal-append-to-body="false" width="300px">
    ...
    <el-dialog title="设备选择" :visible.sync="deviceDialog" :modal-append-to-body="false" width="300px">
        ...
        <span slot="footer" class="dialog-footer" style="text-align:center;">
            <el-button type="primary" @click="addAreaDevices">确 定</el-button>
        </span>
    </el-dialog>
    ...
    methods: {
        loadDevices() {
            this.$ajax.get(this.$URL + '/energy/GetAreaDevices', {
                params: {
                    areaId: this.node ? this.node.key : 0,
        ...
        loadTree() {
            this.$ajax.get(this.$URL + '/energy/GetAreaTree').then((result) => {
                if (result.status === 200 && result.data.success) {
        ...
        deleteArea() {
            ...
            if (ids.length > 0) {
                this.$confirm('此操作将永久删除改能耗区域, 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    this.$ajax.get(this.$URL + '/energy/DeleteAreas', {
                        params: {
                            id: ids
                        }
                    }).then((result) => {
        ...
        saveArea() {
            ...
            this.$ajax.post(this.$URL + '/energy/AddArea', {
        ...
        addAreaDevices() {
            ...
            this.$ajax.get(this.$URL + '/energy/AddAreaDevices', {
                params: {
        ...
        deleteAreaDevice() {
            ...
            this.$ajax.get(this.$URL + '/energy/DeleteAreaDevice', {
                params: {
                    id: this.deleteid
                }
            }).then((result) => {
