...
<el-row :gutter="10" class="box">
    <el-row class="boxheader">
        <el-col :xs="24" :sm="24" :md="5" :lg="5">
            <div class="card headerleft">
                ...
                <div class="line"></div>
                <div>
                    <i class="iconfont icon-lifangti- cursor" style="color:#7AB1EF" @click="changeMain(3)"></i>
                </div>
            </div>
        </el-col>
    ...
    <div :id="'customlabel_'+i" class="customlabel" v-for="(item,i) in customItem" :key="'customlabel_'+i">
        <!-- 数字显示 -->
        <div v-if="list.showType==1" :uuid="'C_'+i+'_'+j" v-for="(list,j) in item" :key="'C_'+i+'_'+j">{{list.DrawLineName}}：{{(labelLineValue&&labelLineValue.hasOwnProperty('C_'+i+'_'+j))?labelLineValue['C_'+i+'_'+j]:'0'}}{{list.DrawLineUnit}}</div>
        <!-- 图标显示 && 状态图标 -->
        <div v-if="list.showType==2&&list.iconType==1" v-for="(list,j) in item" :key="j">{{list.DrawLineName}}：
            <i v-if="((labelLineValue&&labelLineValue.hasOwnProperty('C_'+i+'_'+j))?labelLineValue['C_'+i+'_'+j]:'0')==statusicon.statusVar" :class="statusicon.statusIcon" :style="{color:statusicon.statusIconColor}" v-for="(statusicon,s) in list.statusIcons" :key="s"></i>
        </div>
        <!-- 图标显示 && 动态图标 变色-->
        <div v-if="list.showType==2&&list.iconType==2&&list.animationType==1" v-for="(list,j) in item" :key="j">{{list.DrawLineName}}：
            <i :class="list.Icon" v-if="((labelLineValue&&labelLineValue.hasOwnProperty('C_'+i+'_'+j))?labelLineValue['C_'+i+'_'+j]:'0')==color.statusValue" :style="{color:color.statusColor}" v-for="(color,s) in list.statusColors" :key="s"></i>
        </div>
        <!-- 图标显示 && 动态图标 闪烁 -->
        <div v-if="list.showType==2&&list.iconType==2&&list.animationType==3" v-for="(list,j) in item" :key="j">{{list.DrawLineName}}：
    ...
    <div :id="'linelabel_'+i" class="linelabel" v-for="(item,i) in lineLabel" :key="'linelabel_'+i">
        ...
        <div class="labelpanel">
            <div :uuid="'L_'+i+'_'+j" v-for="(list,j) in item" :key="'L_'+i+'_'+j" v-if="list.showType==1">
            ...
            <!-- 图标显示 && 状态图标 -->
            <div v-if="list.showType==2&&list.iconType==1" v-for="(list,j) in item" :key="j" style="display:flex">{{list.DrawLineName}}：
                <i v-if="((labelLineValue&&labelLineValue.hasOwnProperty('L_'+i+'_'+j))?labelLineValue['L_'+i+'_'+j]:'0')==statusicon.statusVar" :class="statusicon.statusIcon" :style="{color:statusicon.statusIconColor}" v-for="(statusicon,s) in list.statusIcons" :key="s"></i>
            </div>
            <!-- 图标显示 && 动态图标 变色-->
            <div v-if="list.showType==2&&list.iconType==2&&list.animationType==1" v-for="(list,j) in item" :key="j" style="display:flex">{{list.DrawLineName}}：
                <i :class="list.Icon" v-if="((labelLineValue&&labelLineValue.hasOwnProperty('L_'+i+'_'+j))?labelLineValue['L_'+i+'_'+j]:'0')==color.statusValue" :style="{color:color.statusColor}" v-for="(color,s) in list.statusColors" :key="s"></i>
            </div>
            <!-- 图标显示 && 动态图标 闪烁 -->
            <div v-if="list.showType==2&&list.iconType==2&&list.animationType==3" v-for="(list,j) in item" :key="j" style="display:flex">{{list.DrawLineName}}：
...
import "@/assets/bim/jssdk/BimfaceSDKLoader@latest-release";
