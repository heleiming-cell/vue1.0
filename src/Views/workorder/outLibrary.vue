...
<div class="box">
    ...
    <div class="add-new">
        <el-button type="primary" @click="handleAdd" size="small">出库</el-button>
    </div>
    <pagiNation v-show="listQuery.total>0" :total="listQuery.total" :page.sync="listQuery.page" :limit.sync="listQuery.limit" @pagination="getList" />
    <!-- 出库弹框 -->
    <el-dialog title="新增出库" :visible.sync="dialogVisible_spareOut" width="900px">
        ...
        <div slot="footer" class="dialog-footer">
            <el-button @click="dialogVisible_spareOut = false">取 消</el-button>
    ...
    <!-- 出库详情 -->
    <el-dialog title="出库详情" :visible.sync="dialogVisible_details" width="800px">
...
export default {
    ...
    methods: {
        getList() {
            ...
            SpareOut_QueryList(data).then(result => {
        ...
        // 点击出库
        handleAdd() {
            this.dialogVisible_spareOut = true;
        ...
        submitForm() {
            if (this.storageData.length == 0) {
                this.$message.error("请添加要出库的设备")
            } else {
                if (this.storageData.map(v => v.Num == 0).includes(false)) {
                    if (this.storageData.map(v => v.Num > v.Count).includes(true)) {
                        this.$message.error("出库数量不能大于库存数量！")
                    } else {
                        if (this.storageData.map(v => v.Num == 0).includes(true)) {
                            this.$confirm('要出库的备件中有数量为0的数据，该数据将不会被选中出库，是否继续操作？', '提示', {
        ...
        // 请求出库
        postStoreOut(data) {
            SpareOut_Add(data).then(result => {
                this.getList();
                this.$message.success("出库成功");
                this.dialogVisible_spareOut = false
            }).catch(error => {
                console.log(error)
            })
        },
        downFile() {
            ...
            this.$ajax.post(this.$Order + '/sys/excel/QueryListSpareOut', data, {
                responseType: "blob"
            }).then(result => {
                FileSaver.saveAs(
